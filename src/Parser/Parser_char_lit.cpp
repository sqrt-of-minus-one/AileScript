...
void Parser::parse_char_literal()
{
	if (waiting_.empty())
	{
		return;
	...
	std::shared_ptr<CharLiteralToken> token = std::make_shared<CharLiteralToken>();
	...
	case u'Ю':
	{
		if (waiting_[1] == u'\'')
		{
			token->prefix = (waiting_[0] == u'u' || waiting_[0] == u'ю') ? CharLiteralToken::EPrefix::U8 : CharLiteralToken::EPrefix::U32;
			left_quote = 1;
			break;
		}
		else if (waiting_[1] == u'8' && waiting_[2] == u'\'')
		{
			token->prefix = CharLiteralToken::EPrefix::U8;
			left_quote = 2;
			break;
		}
		else if (waiting_[1] == u'1' && waiting_[2] == u'6' && waiting_[3] == u'\'')
		{
			token->prefix = CharLiteralToken::EPrefix::U16;
			left_quote = 2;
			break;
		}
		else if (waiting_[1] == u'3' && waiting_[2] == u'2' && waiting_[3] == u'\'')
		{
			token->prefix = CharLiteralToken::EPrefix::U32;
			left_quote = 2;
			break;
		}
		else [[fallthrough]];
	...
	{
		// Unknown prefix
		token->prefix = CharLiteralToken::EPrefix::INVALID;
